...
import { Company, ClientCredential, ClientHistory, Staff } from '@/types';
import {
  X,
  Shield,
  FileText,
...
interface ProfileProps {
  company: Company;
  staff?: Staff[];
  /** Faol yorliq — URL (`?tab=`) da yashaydi, shuning uchun tashqaridan. */
  activeTab: TabId;
...
const CompanyProfilePanels: React.FC<ProfileProps> = ({ company, staff = [], activeTab, onTabChange, onSave }) => {
    ...
    if (company) {
      ...
      (async () => {
        try {
          ...
          else console.warn('[CompanyProfile] getKpiRules failed:', rulesRes.reason);

          if (compRulesRes.status === 'fulfilled') setCompanyKpiRules(compRulesRes.value);
          else console.warn('[CompanyProfile] getCompanyKpiRules failed:', compRulesRes.reason);
        } finally {
      ...
      <div className="space-y-6 animate-fade-in pb-10">
        ...
        <div className="dashboard-card overflow-hidden">
          ...
          <div className="p-6">
            ...
            <Button variant="secondary" fullWidth onClick={() => onTabChange('xizmatlar')}>
              Barchasini ko&apos;rish
            </Button>
          </div>
        </div>
