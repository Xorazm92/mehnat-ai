...
const SettingsModule: React.FC<Props> = ({ profile, lang, onSaveProfile, onChangePassword }) => {
    ...
    const inputClass = "w-full rounded-lg pl-11 pr-4 py-3 text-xs font-bold outline-none transition-all focus:ring-2 focus:ring-[var(--accent-blue)] focus:ring-opacity-20 tracking-tight";
            ...
            <div className="flex items-center gap-2 p-1 rounded-xl w-fit" style={{ background: 'var(--input-bg)', border: '1px solid var(--card-border)' }}>
                {([['profile', lang === 'uz' ? 'Profil' : 'Профиль'], ['security', lang === 'uz' ? 'Xavfsizlik' : 'Безопасность']] as const).map(([key, label]) => (
                    <button key={key} onClick={() => setTab(key)}
                        className="px-5 py-2 rounded-lg text-meta font-bold uppercase tracking-widest transition-all flex items-center gap-2"
            ...
            {tab === 'profile' && (
                <form onSubmit={saveProfile} className="dashboard-card p-6 space-y-5">
                    ...
                    <div className="space-y-2">
                        <label className="text-micro font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>{lang === 'uz' ? 'F.I.SH' : 'Ф.И.О'}</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="space-y-2">
                            <label className="text-micro font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>{t.phone}</label>
                            <div className="relative">
                        ...
                        <div className="space-y-2">
                            <label className="text-micro font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>{lang === 'uz' ? 'Bo\'lim' : 'Отдел'}</label>
                            <div className="relative">
                    ...
                    <div className="space-y-2">
                        <label className="text-micro font-bold uppercase tracking-widest flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }}><Palette size={12} /> {lang === 'uz' ? 'Avatar rangi' : 'Цвет аватара'}</label>
                    ...
                    <div className="flex justify-end pt-5" style={{ borderTop: '1px solid var(--card-border)' }}>
                        <button type="submit" disabled={savingProfile}
                            className="px-6 py-3 rounded-xl font-bold text-meta text-white transition-all shadow-md hover:shadow-lg uppercase tracking-widest active:scale-95 disabled:opacity-60 flex items-center gap-2"
                ...
                <form onSubmit={savePassword} className="dashboard-card p-6 space-y-5">
                    ...
                    {([['current', lang === 'uz' ? 'Hozirgi parol' : 'Текущий пароль'], ['next', lang === 'uz' ? 'Yangi parol' : 'Новый пароль'], ['confirm', lang === 'uz' ? 'Yangi parolni tasdiqlang' : 'Подтвердите пароль']] as const).map(([key, label]) => (
                        <div key={key} className="space-y-2">
                            <label className="text-micro font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>{label}</label>
                            <div className="relative">
