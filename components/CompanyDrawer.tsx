...
import { Company, OperationEntry, Payment, PaymentStatus, Language } from '../types';
...
const CompanyDrawer: React.FC<DrawerProps> = ({ company, operation, payments, lang, onClose }) => {
  const t = translations[lang];
  const [documents, setDocuments] = useState<any[]>([]);
  const [isLoadingDocs, setIsLoadingDocs] = useState(false);
  const [showPass, setShowPass] = useState(false);
  const [activeTab, setActiveTab] = useState<'general' | 'payments' | 'docs' | 'history'>('general');
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-2xl bg-slate-50 dark:bg-apple-darkBg shadow-[-20px_0_50px_rgba(0,0,0,0.2)] z-[101] overflow-y-auto scrollbar-thin overflow-x-hidden" style={{ animation: 'slideLeft 0.5s cubic-bezier(0.16, 1, 0.3, 1)' }}>
        ...
        <div className="bg-white dark:bg-apple-darkCard border-b border-apple-border dark:border-apple-darkBorder sticky top-0 z-10">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase ${company.isActive ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'}`}>
                    {company.isActive ? 'Faol' : 'Arxiv'}
                  </span>
                </div>
              </div>
            </div>
          ...
          {/* Tabs */}
          <div className="flex px-8 border-t border-apple-border dark:border-apple-darkBorder">
            {[
              { id: 'general', label: 'Umumiy', icon: <Building size={16} /> },
              { id: 'payments', label: 'To\'lovlar', icon: <History size={16} /> },
              { id: 'docs', label: 'Hujjatlar', icon: <FileText size={16} /> },
              { id: 'history', label: 'Audit', icon: <Shield size={16} /> },
            ].map(tab => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as any)}
                className={`flex items-center gap-2 px-6 py-5 border-b-2 transition-all font-black text-xs uppercase tracking-widest ${activeTab === tab.id ? 'border-apple-accent text-apple-accent' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
              >
                {tab.icon} {tab.label}
              </button>
            ))}
          ...
          {activeTab === 'general' && (
            <div className="space-y-10 animate-fade-in">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-6 bg-white dark:bg-apple-darkCard rounded-[2rem] border border-apple-border dark:border-apple-darkBorder shadow-sm">
                  <div className="flex items-center gap-3 mb-4 text-slate-400">
                    <Shield size={16} />
                    <p className="text-[10px] font-black uppercase tracking-widest">Mas'ul Buxgalter</p>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-400">Mas'ul xodim</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 bg-white dark:bg-apple-darkCard rounded-[2rem] border border-apple-border dark:border-apple-darkBorder shadow-sm">
                  <div className="flex items-center gap-3 mb-4 text-slate-400">
                    <Lock size={16} />
                    <p className="text-[10px] font-black uppercase tracking-widest">Ega / Kontakt</p>
                  </div>
                  <div>
                    <p className="font-black text-slate-800 dark:text-white text-lg leading-tight">{company.ownerName || '—'}</p>
                    <p className="text-xs font-bold text-slate-400">Asosiy bog'lanuvchi</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 bg-white dark:bg-apple-darkCard rounded-[2rem] border border-apple-border dark:border-apple-darkBorder shadow-sm">
                <div className="flex items-center gap-3 mb-4 text-slate-400">
                  <Globe size={16} />
                  <p className="text-[10px] font-black uppercase tracking-widest">Tizimga Kirish</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex justify-between items-center px-4 py-3 bg-slate-50 dark:bg-white/5 rounded-xl border border-apple-border dark:border-apple-darkBorder">
                  ...
                  <div className="flex justify-between items-center px-4 py-3 bg-slate-50 dark:bg-white/5 rounded-xl border border-apple-border dark:border-apple-darkBorder relative">
                  ...
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between p-6 bg-white dark:bg-apple-darkCard border border-apple-border dark:border-apple-darkBorder rounded-[1.5rem] hover:ring-2 hover:ring-apple-accent/20 transition-all group">
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">{operation.period} Hisoboti</p>
                      </div>
            ...
            <div className="space-y-8 animate-fade-in">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div className="p-6 bg-emerald-500/5 rounded-3xl border border-emerald-500/10">
                  <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1">To'landi</p>
                  <p className="text-2xl font-black text-emerald-700 tabular-nums">{totalPaid.toLocaleString()} so'm</p>
                </div>
                <div className="p-6 bg-amber-500/5 rounded-3xl border border-amber-500/10">
                  <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1">Kutilmoqda</p>
                  <p className="text-2xl font-black text-amber-700 tabular-nums">{totalPending.toLocaleString()} so'm</p>
                </div>
                <div className="p-6 bg-slate-500/5 rounded-3xl border border-slate-500/10">
                  <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-1">Shartnoma</p>
                  <p className="text-2xl font-black text-slate-700 tabular-nums">{company.contractAmount.toLocaleString()} so'm</p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-apple-darkCard rounded-[2.5rem] border border-apple-border dark:border-apple-darkBorder overflow-hidden">
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-50 dark:bg-white/5 text-[9px] font-black text-slate-400 uppercase tracking-tighter border-b dark:border-apple-darkBorder">
                      <th className="px-6 py-4">Sana</th>
                      <th className="px-6 py-4">Oylik</th>
                      <th className="px-6 py-4">Summa</th>
                      <th className="px-6 py-4 text-right">Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                    {companyPayments.length > 0 ? companyPayments.map(p => (
                      <tr key={p.id} className="text-sm font-bold">
                        ...
                        <td className="px-6 py-4 text-slate-800 dark:text-white uppercase">{p.period}</td>
                        <td className="px-6 py-4 font-black">{p.amount.toLocaleString()}</td>
                        <td className="px-6 py-4 text-right">
                          <span className={`px-2 py-1 rounded-lg text-[10px] uppercase ${p.status === PaymentStatus.PAID ? 'bg-emerald-500/10 text-emerald-600' : 'bg-amber-500/10 text-amber-600'}`}>
                            {p.status}
                          </span>
                        </td>
                      </tr>
                      ...
                      <tr>
                        <td colSpan={4} className="px-6 py-20 text-center text-slate-300 font-bold uppercase text-[10px] tracking-widest">Ma'lumot topilmadi</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          {activeTab === 'docs' && (
            <section className="animate-fade-in">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="h-1 bg-apple-accent rounded-full w-8"></div>
                  <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight">Hujjatlar Arshivi</h3>
                </div>
