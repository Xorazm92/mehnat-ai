...
const acceptedFg = (s: string) => (s === "accepted" || s === "paid" ? "var(--success)" : s === "rejected" || s === "overdue" ? "var(--danger-dark)" : "var(--brand-deep)");
...
export default function PortalClient({ overview, obligations, invoices, requests }: { overview: Overview; obligations: Obl[]; invoices: Inv[]; requests: Req[] }) {
  ...
  const border = { borderColor: "var(--border, var(--rule))" };
        ...
        <div className="overflow-x-auto rounded-xl border" style={border}>
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {obligations.map((o) => (
                <tr key={o.id} className="border-t" style={{ borderColor: "var(--border, var(--bg-sunken))" }}>
                  ...
                  <td className="px-3 py-2" style={{ color: o.isOverdue ? "var(--danger-dark)" : "var(--text-primary)" }}>{formatUzDate(o.dueAt)}</td>
          ...
          {requests.map((r) => (
            <div key={r.id} className={card} style={border}>
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold px-2 py-0.5 rounded-lg" style={{ background: r.status === "answered" ? "var(--success-bg)" : "var(--warning-bg)", color: r.status === "answered" ? "var(--success)" : "var(--warning)" }}>{r.status === "answered" ? "Javob berilgan" : r.status === "closed" ? "Yopilgan" : "Ochiq"}</span>
              </div>
              <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>{r.message}</p>
              {r.responseText && <p className="text-sm mt-2 pl-3 border-l-2" style={{ borderColor: "var(--success)", color: "var(--text-primary)" }}>{r.responseText}</p>}
            </div>
          ))}
        </div>
      </section>
