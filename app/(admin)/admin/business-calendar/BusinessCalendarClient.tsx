...
const KINDS: Record<string, { label: string; isWorkday: boolean; isHoliday: boolean; bg: string; fg: string }> = {
  holiday: { label: "Bayram", isWorkday: false, isHoliday: true, bg: "#fee2e2", fg: "#b91c1c" },
  dayoff: { label: "Dam olish", isWorkday: false, isHoliday: false, bg: "#f3f4f6", fg: "#6b7280" },
  workday: { label: "Ish kuni (override)", isWorkday: true, isHoliday: false, bg: "#dcfce7", fg: "#15803d" },
};
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-1.5">
          {years.map((y) => (
            <button key={y} disabled={pending} onClick={() => reload(y)} className="px-3 py-1.5 rounded-lg text-sm font-semibold disabled:opacity-50" style={{ background: y === year ? "#2563eb" : "var(--bg-hover, #f3f4f6)", color: y === year ? "#fff" : "var(--text-primary)" }}>
              {y}
            </button>
          ))}
        </div>
      ...
      <div className="rounded-xl border p-4 flex flex-wrap items-end gap-3" style={{ borderColor: "var(--border, #e5e7eb)" }}>
        <label className="text-xs" style={{ color: "var(--text-muted)" }}>Sana
          <input type="date" className="block px-2.5 py-1.5 rounded-md border text-sm" style={inputStyle} value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} />
        </label>
        <label className="text-xs" style={{ color: "var(--text-muted)" }}>Turi
          <select className="block px-2.5 py-1.5 rounded-md border text-sm" style={inputStyle} value={form.kind} onChange={(e) => setForm({ ...form, kind: e.target.value as keyof typeof KINDS })}>
        ...
        <label className="text-xs flex-1 min-w-[160px]" style={{ color: "var(--text-muted)" }}>Nomi (ixt.)
          <input className="block px-2.5 py-1.5 rounded-md border text-sm w-full" style={inputStyle} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Mustaqillik kuni" />
        </label>
        <button disabled={pending} onClick={addDay} className="px-4 py-2 rounded-lg text-sm font-semibold text-white disabled:opacity-50" style={{ background: "#16a34a" }}>Saqlash</button>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {days.map((d) => {
                ...
                return (
                  <tr key={d.id} className="border-t" style={{ borderColor: "var(--border, #f1f5f9)" }}>
                    <td className="px-3 py-2.5 font-medium" style={{ color: "var(--text-primary)" }}>{formatUzDate(d.date)}</td>
                    <td className="px-3 py-2.5"><span className="text-xs font-bold px-2 py-0.5 rounded" style={{ background: k.bg, color: k.fg }}>{k.label}</span></td>
                    <td className="px-3 py-2.5" style={{ color: "var(--text-muted)" }}>{d.name || "—"}</td>
                    <td className="px-3 py-2.5 text-right">
                      <button disabled={pending} onClick={() => remove(d.id)} className="text-xs font-semibold px-2 py-1 rounded-md disabled:opacity-50" style={{ background: "#fee2e2", color: "#b91c1c" }}>O'chirish</button>
                    </td>
                  </tr>
                );
              })}
