...
export default function OrganizationsClient({ companies, staff, operations, userRole, tariffPreset, internalContractors, internalParties }: Props) {
  ...
  return (
    <OrganizationModule
      ...
      canCreate={canCreate}
      canDelete={canDelete}
      // Firma kartasi endi ALOHIDA SAHIFA (`/organizations/[id]`).
      // Ilgari bu yerda o'ng tomondan chiqadigan panel ochilardi: to'qqizta
      // yorliq 850px ga sig'masdi, havolasi ulashilmasdi va orqaga tugmasi
      // panelni yopmasdi.
      onCompanySelect={c => router.push(`/organizations/${c.id}`)}
    />
  );
}
