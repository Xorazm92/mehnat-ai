...
async function main() {
  ...
  for (const u of users) {
    ...
    if (/^\d+$/.test(name) || name.includes("jami") || name.includes("naqd") || name === "kpi" || name === "vast") {
      await prisma.user.update({
        where: { id: u.id },
        data: { isActive: false }
      });
      archivedUsers++;
      console.log(`🗑️ Arxivlandi (Xodim): ${u.fullName}`);
    }
  }
  ...
  for (const c of companies) {
    ...
    if (/^\d+$/.test(name) || name.includes("jami") || name.includes("naqd") || name === "kpi" || name === "vast") {
      ...
      archivedCompanies++;
      console.log(`🗑️ Arxivlandi (Firma): ${c.name}`);
    }
  }

  console.log(`\nTozalash tugadi:`);
  console.log(`- ${archivedUsers} ta xato xodim arxivlandi.`);
