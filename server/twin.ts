 ...
 */
async function workFundMinutes(start: Date, end: Date): Promise<number> {
  const days = await prisma.businessCalendarDay.findMany({
...
export async function getStaffCapacity(period: string): Promise<StaffCapacity[]> {
  ...
  const fund = await workFundMinutes(w.periodStart, w.periodEnd);
