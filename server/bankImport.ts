 ...
 */
export async function previewStatement(formData: FormData): Promise<StatementPreview> {
  await requireStatementRole();

  const file = formData.get("file");
  if (!(file instanceof File)) throw new Error("Fayl yuborilmadi");
  ...
  return {
    ...
    expenseSum: expense.reduce((s, t) => s + t.amount, 0),
    duplicateCount,
...
export async function commitStatementUpload(formData: FormData) {
  ...
  return serialize({ ...result, account: account.label, match });
}
